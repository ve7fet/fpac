...
//
		...
		sockaddr.rose.srose_ndigis = 0;
// DEBUG F6BVP
		node_msg("ROSE User.dl.name : %s  family AF_ROSE ndigis = 0", User.dl_name);
//
...
// DEBUG F6BVP
		node_msg("ROSE AF_ROSE address : %s via : %s", path, address[pos]);
...
// DEBUG F6BVP
		node_msg("ROSE AF_AX25 | AF_FLEXNET ? %s", family);
//
...
FSA*/
                if ((dest = ax25_config_get_addr(address[0])) == NULL) {
                    node_msg("Connect to invalid port");
                    return -1;
                }
		if (strcasecmp(address[0], cfg.alt_callsign) == 0)
		{
...
//
// Here we finally ask Linux for connection !
//
...
/*			argv[k++] = ax25_config_get_name(flgt->dev); */
	...
	/* set protocole family */
		...
		{
			node_msg("Connect port missing : Connect port callsign");
			wp_close();
			return (0);
		}
		wp_close();
...
// DEBUG F6BVP
//			node_msg("AX.25 port callsign - %s", argv[2]);
//			family = AF_AX25;
//			source = "(user port) ";
//
//
