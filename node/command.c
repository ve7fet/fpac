...
int do_ports(int argc, char **argv)
{
	...
	if (Colored)
		node_msg("Ports:\n%sPort%s\t%sDev%s\t %sDescription%s", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
	else
	...
	for (p = list; p != NULL; p = p->next)
	{
		...
		if (first)
		{
			...
			if (Colored)	
				...
				F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
			else
			...
			if (is_sysop())
			{
				if (Colored)
					...
					F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
				else
...
/*		if (rp->lci > ROSE_DEFAULT_MAXVC)
		...
		if (first)
		{
			...
			if (Colored)	
				tprintf("%sCallsign%s  %sDNIC addr%s   <-> %sCallsign%s  %sDNIC addr%s   %sLCI Adjacent%s    %sAX.25 state%s", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
			else
				tprintf
			...
			{
				if (Colored)
					tprintf("    %sUnack Snd-Q Rcv-Q%s", F_Yellow, ResetColor);
				else
...
*/
	...
	for (pn = listn; pn != NULL; pn = pn->next)
	{
		...
		{
			if (Colored)
				node_msg("ROSE routes :\n%sDNIC Address%s %sPrimary   Route%s  | %s1st Alt   Route%s  | %s2nd Alt   Route%s  |", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
			else
...
int do_links(int argc, char **argv)
{
	...
	list = read_proc_ax25();
	if (Colored)
		node_msg("Adjacent Nodes Links:\n%sCallsign%s  %sStatus%s       %sDev%s    %sIface%s  %sPort%s", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
	else
...
*/
	...
	if (argc == 1)
	{
		node_msg("FPAC Nodes:");
		if (Colored)
			node_msg("%sCallsign%s  %sDNIC addr%s    %sCallsign%s  %sDNIC addr%s    %sCallsign%s  %sDNIC addr%s", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
		else
...
int do_netrom(int argc, char **argv)
{
	...
	if (argc == 1)
	{
		...
		if (Colored)
			node_msg(" %sAlias:Callsign Qual.%s   %sAlias:Callsign Qual.%s   %sAlias:Callsign Qual.%s", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
		else
...
//	tprintf("FPAC White Pages database : %d callsigns\n", wp_nb_records());
	...
	if (wp_get_list(&wp, &nb, flags, argv[optind]) != -1)
	{
		for (i = 0; i < nb; i++)
		{
			...
			{
				if (Colored)
					tprintf(" %sCallsign%s   %sLast update UTC%s   %sDNIC address%s  %sN/U\t Digi%s \t  %sLocator City%s\n", F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor, F_Yellow, ResetColor);
				else
